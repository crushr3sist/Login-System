class signin:
    ...
    def login(self):
        ...
        if (
            self.login_Password not in self.fetchAuthData
            or self.login_Username not in self.fetchAuthData
        ):
            for i in range(0, 6):
                ...
                if (
                    ...
                    if i >= 5:
                        ...
                        break
        if (
            self.login_Password in self.fetchAuthData
            and self.login_Username in self.fetchAuthData
        ):
            print("Login Sucessful")
